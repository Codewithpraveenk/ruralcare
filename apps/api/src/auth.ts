...
export type Role="CITIZEN"|"ASHA"|"STAFF";
export type SessionUser={id:string;name:string;email:string|null;phone:string|null;role:Role;facilityId:string|null};
...
export function issueSession(res:Response,user:SessionUser){const token=jwt.sign({sub:user.id,name:user.name,email:user.email,phone:user.phone,role:user.role,facilityId:user.facilityId},secret(),{expiresIn:"8h",issuer:"ruralcare-connect",audience:"ruralcare-web"});res.cookie(cookieName,token,{httpOnly:true,secure:process.env.COOKIE_SECURE==="true",sameSite:"lax",maxAge:8*60*60*1000,path:"/"});}
