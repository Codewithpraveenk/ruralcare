...
const stages = ["CREATED", "ACCEPTED", "ARRIVED", "FOLLOW_UP_DUE", "COMPLETED"] as const;
const stageLabel = (stage: string) => stage === "FOLLOW_UP_DUE" || stage === "FOLLOW_UP" ? "Follow-up due" : stage.slice(0, 1) + stage.slice(1).toLowerCase();
...
function coordination() {
  ...
  return { cases, totals: { incoming: cases.length + 8, urgent: cases.filter((item) => item.urgency !== "ROUTINE").length, pending: cases.filter((item) => ["CREATED", "ACCEPTED"].includes(String(item.status))).length, followups: cases.filter((item) => item.status === "FOLLOW_UP_DUE" || item.status === "FOLLOW_UP" || Boolean(item.followUpDue)).length }, demand, capacityAlerts: [{ title: "Primary care unavailable at Public Health Centre", service: "PRIMARY_CARE", affected: affected.map((item) => ({ demoId: item.demoId, careNeed: item.careNeed })), alternatives }], activity: cases.slice(0, 5).map((item) => ({ demoId: item.demoId, stage: item.stage, careNeed: item.careNeed, facility: item.facility, updatedAt: item.updatedAt })) };
}
...
app.patch("/api/referrals/:id", async (req, res) => {
  const status = z.enum(["CREATED", "ACCEPTED", "ARRIVED", "FOLLOW_UP_DUE", "COMPLETED"]).safeParse(req.body.status);
