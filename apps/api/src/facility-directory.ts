import { calculateDistanceKm, type Facility, type Service } from "@ruralcare/shared";
...
export function buildFacilityRecords(): Facility[] {
  return coordinateEnrichments.map((enrichment) => {
    ...
    return { id: enrichment.facilityId, name: source.name, type: enrichment.routingLevel, services: explicitServices.length ? explicitServices : referenceServices[enrichment.routingLevel], available: availability.available, hours: availability.note, address: source.address, phone: "Not published in supplied directory", latitude: enrichment.latitude, longitude: enrichment.longitude, distanceKm: calculateDistanceKm(demoOrigin, enrichment) };
  });
}
