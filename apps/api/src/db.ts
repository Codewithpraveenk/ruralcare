...
export function initializeDatabase() {
  ...
  const authColumns=db.prepare("PRAGMA table_info(User)").all() as Array<{name:string;notnull:number}>;
  if(!authColumns.some(item=>item.name==="googleSub")||authColumns.find(item=>item.name==="passwordHash")?.notnull===1)db.exec(`BEGIN;
