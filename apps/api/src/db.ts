...
export function initializeDatabase() {
  db.exec(`CREATE TABLE IF NOT EXISTS Facility (id TEXT PRIMARY KEY, name TEXT NOT NULL, type TEXT NOT NULL, distanceKm REAL NOT NULL, services TEXT NOT NULL, available INTEGER NOT NULL, hours TEXT NOT NULL, address TEXT NOT NULL, phone TEXT NOT NULL, latitude REAL NOT NULL, longitude REAL NOT NULL, capacityData TEXT NOT NULL DEFAULT '{}', lastUpdated TEXT NOT NULL DEFAULT 'Synthetic demo shift');
    ...
    CREATE TABLE IF NOT EXISTS ReferralStatusEvent (id TEXT PRIMARY KEY, referralId TEXT NOT NULL, fromStatus TEXT, toStatus TEXT NOT NULL, timestamp TEXT NOT NULL, actorType TEXT NOT NULL);
  ...
  for (const column of [["clientId", "TEXT"], ["demoId", "TEXT"], ["careNeed", "TEXT"], ["followUpDue", "TEXT"], ["updatedAt", "TEXT"], ["requestId", "TEXT"], ["sourceMode", "TEXT"], ["selectedFacilityType", "TEXT"], ["routingExplanation", "TEXT"], ["rerouted", "INTEGER DEFAULT 0"], ["previousFacility", "TEXT"], ["routingAudit", "TEXT"], ["rerouteStatus", "TEXT"], ["recommendedFacility", "TEXT"], ["rerouteTimestamp", "TEXT"]]) if (!columns.some((item) => item.name === column[0])) db.exec(`ALTER TABLE Referral ADD COLUMN ${column[0]} ${column[1]}`);
