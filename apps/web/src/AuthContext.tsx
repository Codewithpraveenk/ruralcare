...
type AuthValue={user:AuthUser|null;loading:boolean;error:string;login:(identifier:string,password:string)=>Promise<void>;register:(input:Registration)=>Promise<void>;logout:()=>Promise<void>;refreshUser:()=>Promise<void>;clearError:()=>void};
const AuthContext=createContext<AuthValue|null>(null);
async function authRequest(path:string,init?:RequestInit){const response=await fetch(path,{credentials:"include",headers:{"Content-Type":"application/json"},...init}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||"Authentication request failed.");return data;}
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const login=async(identifier:string,password:string)=>{setError("");try{const result=await authRequest("/api/auth/login",{method:"POST",body:JSON.stringify({identifier,password})});setUser(result.user);}catch(reason){const message=reason instanceof Error?reason.message:"Login failed.";setError(message);throw reason;}};
 ...
 const value=useMemo(()=>({user,loading,error,login,register,logout,refreshUser,clearError:()=>setError("")}),[user,loading,error]);return <AuthContext.Provider value={value}>{children}</AuthContext.Provider>;
}
